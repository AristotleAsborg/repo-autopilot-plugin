...
> 单个 issue：M2 分类 → 视情进 M3　·　引用：阶段三 3.1–3.3 / 阶段四　·　含写操作：是
...
1. 拉取 issue 原文（读操作直通）；先进查重（3.2，命中就提示重复，不重复分类）。
2. 分类打标 + 首条回复模板（3.1）；置信度 <0.7 只发建议评论，不动标签。
3. 打回判定（3.3）：预检不过就退回并记账，不进入修复。
4. 判为缺陷 → 走 `/修复` 的主流程（定位 → 修复循环 → 门禁）。
5. 产出报告并调用 `require_human_approval()`；**未获「是」之前禁止任何写操作**。
