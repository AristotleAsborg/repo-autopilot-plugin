// 本文件由 `scripts/build_module.py` 从 `host.js` **生成** —— 不要手改。
  ...
  apply(ctx) {
    ...
    const MODES = {
      doctor: {
        ...
        // 1 **是预期内的一档**（2026-09-20 实测报告 ①）：doctor 退出 1 表示
        // "七项里有不合格项"，而那常常是**环境缺口**而不是仓库损坏
        // （只读副本按设计不带写 token/读 token）。原来只写 [0]，
        // 于是这一档落到"（退出码不在预期集合内）"——把一句真话讲成了怪话。
        ok: [0, 1],
        verdict: (code) =>
          code === 0
            ? '七项自检全过'
            : code === 1
              ? '七项里有不合格项（**可能是环境缺口**，例如只读副本没有写/读 token）'
              : '自检没能跑完，见 summary',
        // **把不合格项提到结论里**（同一次实测）：工具只回 tail（末 25 行），
        // 而"哪几项不合格"往往在前面 —— 那次尾巴上全是通过项，不合格的两项被切走，
        // 模型拿到的是通用兜底文案而不是"七项里 2 项不合格：写 token 文件、GitHub 连通性"。
        // 退出码非 0 时 stdout 就是那份 JSON，摘要是廉价且可靠的。
        digest: (stdout) => {
          let data
          try {
          ...
          const items = Array.isArray(data)
            ...
            : data && Array.isArray(data.checks)
              ? data.checks
              : []
          const bad = items.filter((item) => item && item.ok === false)
          if (bad.length === 0) return ''
          return (
            '不合格 ' +
            String(bad.length) +
            ' 项：' +
            bad.map((item) => text(item.name, '(未命名项)')).join('、')
          )
        },
        timeoutMs: 120000,
      },
    ...
    // 自包含（同 DEFAULT_REPO_ROOT 的道理，2026-09-20）：**安装时验证过的解释器**。
    //
    // 为什么必须"写进来"：本机默认的 `python` 没装依赖，而"会话内设环境变量"这条路
    // 已被真机复现两次走不通 —— DSH 会话里的 shell 继承的是 DSH 进程的环境，
    // 会话里改不了 `REPO_AUTOPILOT_PYTHON`。于是每次调用都得由人类/模型**手传 `python`**，
    // 这是常态化的摩擦。而安装脚本**本来就知道**哪个解释器能 import 依赖
    // （它自己就是用那个解释器跑自检的），没有理由让运行时再去盲探。
    //
    // 发布版这里是空串 → 候选表自动跳过它、退回盲探，行为与之前完全一致。
    ...
    const PY_CANDIDATES = [
      ...
      // 安装时烘入的那个 —— 放在盲探之前：它**已知**能 import 依赖，
      // 而 `python3`/`python` 只是碰运气（本机就是碰不到的那个）。
      // 仍然照常探测：烘入之后环境变了也要能发现，而不是盲信。
      ...(DEFAULT_PYTHON
        ? [{ expr: DEFAULT_PYTHON, raw: false, label: '安装时烘入的解释器（已验证依赖）' }]
        : []),
    ...
    async function runMode(args, signal) {
      ...
      // 摘要只在"确实是仓库/环境的结论"时才算数 —— 是我们自己拼错命令时，
      // 说"哪几项不合格"会把人引到错误的方向（第一轮就吃过这个亏）。
      const digest = misdiagnosis === null && mode.digest ? mode.digest(result.stdout.text) : ''
      const verdict =
        misdiagnosis === null
          ? mode.verdict(result.exitCode) +
            (flag === true ? '' : '（退出码不在预期集合内）') +
            (digest ? '；' + digest : '')
